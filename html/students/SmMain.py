...
def WriteOnePitchToLocalPitchTable():
    ...
    WriteLogFile('Machine ID at start of WriteOnePitchToLocalPitchTable '  + ThisSess_MachineId, 'WriteOnePitchToLocalPitchTable')
...
# 8/12/19 htc added global values for "hit" area in practice mode.  Needs to pass all the way up to EasyInsight.  Hit/miss still on COLOR (Green).
    ...
    smartmitt_conn = database.get_db_connection()
...
#second arg is session obj, future state is all session columns will be stored in this obj
def RunStandardAndPracticeSession(CurrentSessNoFromArgs, session):
    WriteLogFile(str(session.__dict__), 'main')
...
##  display target
        ...
        if ThisSess_Type.upper() == 'PRACTICE':
            ...
            if CurrFrame_SuccessCount >= CurrFrame_MoveOnSuccessCount or CurrFrame_MissCount >= CurrFrame_MoveOnMissCount or CurrFrame_TotalCount >= CurrFrame_MoveOnTotalCount:
                ...
                smartmitt_conn_pracframe = database.get_db_connection()
...
#9/19/19 htc  added Practice Mode "normal end" (Counter limit reached).     ThisSess_ShutdownMode
                    ...
                    if KeepProcessingPitches:
                        ...
                        TargetToDefine = framerow[6]
                        #   TRF added to try t get a different target to display    TargetToDefine = framerow[15]
                        smartmitt_conn_pracframe.close()
                        gc.collect()
    ...
    # 9/9/18 htc - no matter what - set the time the SpeedDisplayTimerSecs will "expire".
            ...
            if ReturnSpeedValue > 10 and ReturnSpeedValue < 110:
                ...
                if ThisSess_DisplaySpeedSecs > 0:
                    ...
                    if (session.showPlateSpeed):
                        DisplaySpeed(ReturnSpeedValue, SM_Red_Med, 'PLATE SP')
                        time.sleep(1)
                    if (session.showReleaseSpeed and int(ThisSess_ReleaseSpeedAddon) > 0 and int(ThisSess_ReleaseSpeedAddon) < 10):
...
# 11/30/18 htc added process BALL CENTERS sub-routine for "Z1" calculations.
                    ...
                    if os.path.isfile('/home/pi/front.png'):  # os.system('sudo chmod 777 ' + '/home/pi/front.png')
                        ...
                        os.system('cp /home/pi/front.png ' + ThisSessionFolderName + '/' + str(CurrPitch_PitchNumber).zfill(3) + 'front.png')
                        os.system('cp /home/pi/front.png ' + SessionsLocalSaveFolder + '/front.png')
...
#01/05/19 htc                         CmdCallResult = os.system('sudo rm /home/pi/front.png')
                    ...
                    if os.path.isfile('/home/pi/side.png'):  # os.system('sudo chmod 777 ' + '/home/pi/front.png')
                        ...
                        os.system('cp /home/pi/side.png ' + ThisSessionFolderName + '/' + str(CurrPitch_PitchNumber).zfill(3) + 'side.png')
                        os.system('cp /home/pi/side.png ' + SessionsLocalSaveFolder + '/side.png')
...
#01/05/19 htc                        CmdCallResult = os.system('sudo rm /home/pi/side.png')
                    ...
                    if os.path.isfile('/home/pi/top.png'):  # os.system('sudo chmod 777 ' + '/home/pi/front.png')
                        ...
                        os.system('cp /home/pi/top.png ' + ThisSessionFolderName + '/' + str(CurrPitch_PitchNumber).zfill(3) + 'top.png')
                        os.system('cp /home/pi/top.png ' + SessionsLocalSaveFolder + '/top.png')
...
def __main__():
  ...
  #      print ('done playing sound')
    ...
    ThisSess_MachineId = database.read_machine_id()

    WriteLogFile('Machine ID (UnitSerialNumber) from local smconfig table: ' + ThisSess_MachineId, '0001-Main -                   -')

    Build1260ArrayStucture()   #   create blank array
    MarqueeWord(ThisSess_MachineId, '')
    WriteLogFile('Machine ID after Marquee '  + ThisSess_MachineId, 'main')
    Write1260ArrayToArtnetFile() #   Write array to NdbArtNetCtrlString.txt file
    ArtdmxToNDB()   # play current NdbArtNetCtrlString.txt on disk in home folder to NDB/LED 10.0.0.100
...
# 9/9/19 htc added software version and MachineID to Marquee display for a few seconds on EVERY BOOT-UP.
        ...
        WriteLogFile('Session number' + str(RunThisSessionNumber), 'main')
        session = database.read_session(RunThisSessionNumber)
        WriteLogFile(str(session.__dict__), 'main')

        if str(ThisSess_Type).upper() == 'STANDARD' or str(ThisSess_Type).upper() == 'PRACTICE':
            RunStandardAndPracticeSession(RunThisSessionNumber, session)
